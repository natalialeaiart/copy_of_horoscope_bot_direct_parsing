...
class HoroscopeBot:
    ...
    async def run_bot(self) -> None:
        try:
            ...
            # Для Render: создаем простой HTTP-сервер, чтобы сервис не падал
            # из-за отсутствия открытых портов
            ...
            asyncio.create_task(self._start_polling(application))
            
            # Запускаем веб-сервер, чтобы Render не останавливал сервис
            runner = web.AppRunner(app)
            await runner.setup()
            site = web.TCPSite(runner, '0.0.0.0', port)
            await site.start()
            
            logger.info(f"Bot started in polling mode with web server on port {port}")
    ...
    async def _start_polling(self, application):
        try:
